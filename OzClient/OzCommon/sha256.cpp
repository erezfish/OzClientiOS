...
#define SHA_FAILURE		0

	int SHA256_digest(char* input, char* output)
	{
		int result = SHA_FAILURE;
		SHA256_CTX ctx;
		...
		if(input != NULL) {
		   ...
		   result = SHA256_Final((unsigned char*)output, &ctx);
...
int SHA256_digest(char* input, char* output)
{
   ...
   sha256_init(&ctx);
   sha256_update(&ctx, (uchar*)input, strlen(input));
   sha256_final(&ctx, (uchar*)output);
